...
- [x] 5.4 Implement scoped `table_exact_lookup` call in ask.py using `candidate_source_id` (only when not None); prepend result as `## Verified Table Data` section
...
- [x] 5.10 Write integration test: end-to-end ask.py with a CSV source, exact-match query → answer grounded in table data
...
- [x] 8.1 Run full existing test suite; confirm zero regressions on Phase 1 functionality
- [x] 8.2 Verify `source_chat` exact lookup still works correctly with the upgraded two-phase `table_exact_lookup`
- [x] 8.3 Verify `source.tables_markdown` is still populated correctly for new ingestions with Phase 2 safety caps active
